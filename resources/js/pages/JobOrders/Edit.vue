...
import { ClipboardSignature, Plus, Trash, CheckCircle, Save } from 'lucide-vue-next';
...
import { displayJobOrders } from '@/routes';
...
// Form state
const form = useForm({
    customer_name: props.jobOrder.customer_name,
    customer_phone_num: props.jobOrder.customer_phone_num,
    vehicle_plate: props.jobOrder.vehicle_plate,
    vehicle_brand: props.jobOrder.vehicle_brand,
    vehicle_model: props.jobOrder.vehicle_model,
    vehicle_picture: null as File | null,
    reported_issue: props.jobOrder.reported_issue,
    status: props.jobOrder.status,
    total_cost: Number(props.jobOrder.total_cost),
    parts_used: props.partsUsed.map(part => ({
        job_order_parts_id: part.job_order_parts_id,
        automotive_parts_id: part.automotive_parts_id.toString(),
        variation_id: part.variation_id ? part.variation_id.toString() : null,
        quantity: part.quantity,
        unit_price: Number(part.unit_price),
        subtotal: Number(part.subtotal),
        original_quantity: part.original_quantity,
    })),
});
...
const addPartRow = () => {
    form.parts_used.push({
        job_order_parts_id: null as any,
        automotive_parts_id: '',
        variation_id: null,
        quantity: 1,
        unit_price: 0,
...
// Get maximum available stock for a part row (including original quantity for editing)
const getMaxStock = (index: number) => {
    ...
    // Calculate available stock including the original quantity used
    const originalQty = row.original_quantity || 0;
    
    // If variation is selected, return variation stock + original quantity
    if (row.variation_id) {
        const variation = part.variations?.find(v => v.variation_id.toString() === row.variation_id);
        return (variation?.stock_quantity || 0) + originalQty;
    }
    
    // Otherwise return base part stock + original quantity
    return (part.stock_quantity || 0) + originalQty;
};
...
const onPartSelect = (index: number) => {
    ...
    if (part) {
        ...
        form.parts_used[index].original_quantity = 0; // New part, no original quantity
        updateRowSubtotal(index);
    }
};
...
const existingVehiclePicture = ref<string | null>(props.jobOrder.vehicle_picture ? `/storage/${props.jobOrder.vehicle_picture}` : null);
...
const handleVehiclePictureUpload = (event: Event) => {
    ...
    if (file) {
        ...
        reader.onload = (e) => {
            ...
            existingVehiclePicture.value = null; // Hide existing picture when new one is uploaded
        };
...
const removeExistingPicture = () => {
    existingVehiclePicture.value = null;
    // Note: We don't actually delete from server until form is submitted
};

const submit = () => {
    form.put(`/job-orders/update/${props.jobOrder.job_orders_id}`); 
};
...
<template>
    ...
    <div class="my-6 px-12 flex items-start gap-4">
        ...
        <div>
            <h1 class="text-2xl font-bold tracking-tight">Edit Job Order</h1>
            <p class="text-gray-500 mt-1">Update job order details and parts used.</p>
        </div>
    </div>
    ...
    <div class="mb-10 px-4 w-full">
        <form class="max-w-6xl mx-auto p-8 rounded-xl shadow-sm border bg-card text-card-foreground" @submit.prevent>
            <FieldGroup>
                ...
                <FieldSet>
                    ...
                    <FieldGroup>
                        ...
                        <Field>
                            ...
                            <div v-if="existingVehiclePicture" class="mt-4 relative inline-block">
                                <img :src="existingVehiclePicture" alt="Current vehicle" class="w-48 h-32 object-cover rounded-lg border" />
                                <button 
                                    @click="removeExistingPicture" 
                                    type="button"
                            ...
                            <div v-if="vehiclePicturePreview" class="mt-4 relative inline-block">
                                ...
                                <p class="text-xs text-emerald-600 mt-1">New image</p>
                            </div>
                        </Field>
                    </FieldGroup>
                </FieldSet>
                ...
                <FieldSet>
                    <FieldLegend>Parts Used</FieldLegend>
                    <FieldDescription>Update parts required for this job. Stock will be adjusted based on quantity changes.</FieldDescription>
                    <FieldGroup>
                        <div 
                ...
                <div class="flex justify-end bg-muted/40 p-6 rounded-lg border">
                    <div class="flex flex-col items-end gap-2">
                        <span class="text-sm font-medium text-gray-500 uppercase tracking-wide">Total Cost</span>
                ...
                <div class="flex items-center justify-end gap-4 mt-8 pt-6 border-t">
                    <Button variant="outline" class="w-full sm:w-auto" as-child>
                        <Link :href="displayJobOrders().url">
                            Cancel
                        </Link>
                    </Button>
                    <AlertDialog>
                        <AlertDialogTrigger as-child>
                            ...
                            >
                                <Save class="w-4 h-4" /> Update Job Order
                            </Button>
                        </AlertDialogTrigger>
                        <AlertDialogContent>
                            <AlertDialogHeader>
                                <AlertDialogTitle>Update Job Order?</AlertDialogTitle>
                            </AlertDialogHeader>
                            <AlertDialogFooter>
